...
resource "aws_wafv2_web_acl" "web-acl" {
  ...
  rule {
    name     = "allow-foo--waf-acme-lb-example-com---webhooks---viber"
    priority = 2
    statement {
        and_statement {
            statement {
              regex_match_statement {
                regex_string = "foo.waf-acme-lb.example.com"
                field_to_match {
                  single_header {
                    name = "host"
                  }
  ...
  rule {
    name     = "allow-bar--waf-acme-lb--example--com---webhooks---facebook---webhook"
    priority = 3
    statement {
        and_statement {
            statement {
              regex_match_statement {
                regex_string = "bar.waf-acme-lb.example.com"
                field_to_match {
                  single_header {
                    name = "host"
                  }
            ...
            statement {
              byte_match_statement {
                search_string = "/webhooks/facebook/webhook"
                field_to_match {
                  uri_path {}
                }
                text_transformation {
